...
{
	m_image=::LoadImage(::AfxGetInstanceHandle(),MAKEINTRESOURCE(bitmapid), IMAGE_BITMAP,0,0,LR_LOADMAP3DCOLORS);
	mybitmap.Attach(m_image);
	BITMAP	bitmapbits;
...
void CHoverButton::SetCheckType(BOOL nType)
{
	m_bCheck = nType;
}

BOOL CHoverButton::bIsChecked()
{
	return !!m_bCheck;
}